...
void CWorkflow::RunCommandLine(STransportParams* transportParams, CWnd* wndNotify, bool playerStatus, SGammaThresholds* gammaThresh)
{
  m_transportParams = transportParams;
  m_wndNotify       = wndNotify;
  m_playerStatus    = playerStatus;
  m_errorMsg        = WF_ERROR_NONE;
  m_gammaThresholds = gammaThresh;
...
bool CWorkflow::BuildJSON(void)
{
  ...
  CString convertName = m_transportParams->fileNameDose.IsEmpty() ? m_transportParams->fileNamePlan : m_transportParams->fileNameDose;

  if (m_pacsPipe->ConvertStudyDicom2Json(m_transportParams->pathNamePlan, convertName) == false)
  {
